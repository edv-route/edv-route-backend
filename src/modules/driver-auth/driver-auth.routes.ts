...
const driverAuthRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/payment-submissions', { onRequest: [app.authenticateDriver] }, async (req, reply) => {
    ...
    // The app pays the alta (`enroll`, membership + N weeks), settles `debt`, or
    // prepays weeks while up to date (`advance`, opened to the app 2026-08-21 —
    // an affiliate with zero debt had no way to pay anything). `change_plan`
    // stays admin-only: choosing a different tariff is a commercial decision.
    const rawPurpose = fields['purpose'] ?? 'debt';
    if (rawPurpose !== 'debt' && rawPurpose !== 'enroll' && rawPurpose !== 'advance') {
      throw app.httpErrors.badRequest('Tipo de pago no permitido desde la app');
    }
    const purpose = rawPurpose as 'debt' | 'enroll' | 'advance';
