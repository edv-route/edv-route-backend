...
| Método | Ruta | Descripción |
|---|---|---|
| POST | `/drivers/:id/payment-submissions` | Crea un envío **pendiente** (multipart). Campos: `purpose` (`debt`\|`advance`\|`enroll`\|`change_plan`), `periods?` (advance/enroll/change_plan), `planId?` (**change_plan**: nueva tarifa), datos del pago (`paymentMethodId?`, `reference?`, `payerBank?`, `paidOn?`, `payerPhone?`, `payerId?`, `payerAccount?`), `note?`, `amountUsd?` (solo **Efectivo Divisa** `cash_usd`), y **1..5 imágenes** en el campo `files` (PDF/JPG/PNG, 10 MB c/u, magic-number; para `cash_usd` = fotos de billetes). `debt` = salda la deuda actual; `advance` = adelanta N semanas de la tarifa vigente; `enroll` = alta (membresía + N semanas); `change_plan` = cambia de tarifa (`immediate` si estaba vencida, si no `scheduled`) y prepaga N semanas. Todo `purpose` emite **una** factura al aprobar. 409 si el chofer ya tiene un envío pendiente. Origen `admin` hoy; la **app del chofer** POSTea aquí con su token `driver` |
