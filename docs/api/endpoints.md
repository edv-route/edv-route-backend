...
| POST | `/drivers/:id/approve` | Aprobar (exige membresía `paid` + tarifa **y deuda 0**; 409 si falta algún pago o si arrastra deuda — decisión 2026-07-29). La tarifa comienza a correr; el chofer queda `approved` + disponible `is_available = true` |
...
| POST | `/payment-submissions/:id/reverse` | **Revierte** un recibo **aprobado**: `{ reversalType: 'refund'\|'correction', reason }`. *refund* anula sus facturas; *correction* devuelve a deuda lo saldado (para re-cobrar). Marca `reverted` con rastro; si pierde la membresía, el chofer vuelve a `pending`. 409 si no está aprobado. 204 |
