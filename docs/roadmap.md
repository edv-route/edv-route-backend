...
> Actualizado: 2026-07-27. **La lógica del módulo admin está COMPLETA** y **desplegada en
> producción (Railway)** — backend + frontend, ver
> [guides/deploy-railway.md](guides/deploy-railway.md). En curso: **pulido incremental de la UI
> sección por sección** (formularios, componentes de marca); pendiente el rediseño visual completo.
> Después, módulos post-admin.
...
| Historiales | Vista `v_driver_payments` · `GET /invoices` + `GET /payments` (filtros, `driverId`) · pantalla Facturación global (tabs Facturas/Pagos) · **historial de pagos por chofer** (`/drivers/:id/payments`, botón en el detalle: lista + detalle con comprobante, reutiliza los mismos endpoints) | ✅ |
...
| Componentes de marca (UI) | `shared/components/select` (desplegable custodiado con teclado y ARIA) · `shared/components/password-input` (ojo mostrar/ocultar) · `shared/directives/password-policy` · validación visible global (`.ng-invalid.ng-touched`) · `cursor:pointer` global · **modal de confirmación con loading** para acciones importantes (suspender/pausar/reanudar/reactivar) y logout · **visor de archivos en modal** (`shared/components/file-viewer`, PDF/imagen) que reemplaza abrir en pestaña · desplegables de marca en todo el panel (fuera los `<select>` nativos) | ✅ |
| Avisos al afiliado | **Buzón transaccional + bandeja + campana** (2026-08-20): el aviso se escribe dentro de la transacción del hecho y un despachador aparte lo entrega. 15 avisos automáticos de dinero y aprobación, `GET/POST /driver-auth/me/notifications`, contador dentro de `/me/account` y campana en el header de la app. Push real por **FCM HTTP v1** sin SDK (la cuenta de servicio firma el JWT, `fetch` hace el resto), con dos candados para que un backend local no mande push a choferes reales. **ENCENDIDO en producción** desde el 2026-08-21 (push reales verificados en el teléfono). Detalle en [features/notifications.md](features/notifications.md) | ✅ |
| Recuperar la clave (app) | **«Olvidé mi clave» completo y EN PRODUCCIÓN** (2026-08-24): cédula + correo (deben coincidir con el mismo afiliado) → código de 6 dígitos por correo → clave nueva. Código **hasheado** en la BD, 10 min, 3 intentos, 5 por hora y 60 s entre envíos; verificar el código entrega un token de **un solo propósito** que no sirve como sesión. El correo sale por la **API de Gmail** (HTTPS) porque **Railway bloquea el SMTP saliente**; Resend queda implementado para el día que exista un dominio propio. 4 pantallas en la app + el enlace del login, que hasta hoy decía «próximamente». Verificado de punta a punta contra producción | ✅ |
| Correo obligatorio | El correo es **obligatorio al registrarse en los dos canales** y ya no se puede vaciar en ninguna edición (2026-08-24): es la llave de la recuperación de clave, así que dejarlo opcional era dejar gente sin puerta de vuelta. La regla vive en el **backend**, no en cada cliente | ✅ |
| Despliegue | **En producción (Railway)**: backend (Nixpacks) + frontend (Docker/Caddy) como dos servicios del mismo proyecto, apuntando al **mismo Supabase que dev** · CORS por lista de orígenes exactos · health `/api/v1/health` · runbook y gotchas en [guides/deploy-railway.md](guides/deploy-railway.md) | ✅ |
...
2. **Módulos post-admin** (pospuestos conscientemente): apps móviles, viajes/subasta,
   clientes, tickets, Supabase Auth. (**push real: hecho** el 2026-08-20.)
