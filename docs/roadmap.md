...
| Avisos al afiliado | **Buzón transaccional + bandeja + campana** (2026-08-20): el aviso se escribe dentro de la transacción del hecho y un despachador aparte lo entrega. 15 avisos automáticos de dinero y aprobación, `GET/POST /driver-auth/me/notifications`, contador dentro de `/me/account` y campana en el header de la app. Push real por **FCM HTTP v1** sin SDK (la cuenta de servicio firma el JWT, `fetch` hace el resto), con dos candados para que un backend local no mande push a choferes reales. Detalle en [features/notifications.md](features/notifications.md) | ✅ |
